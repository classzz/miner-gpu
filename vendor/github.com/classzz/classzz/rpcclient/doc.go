...
czzwallet provide a websocket interface that is more efficient than the standard
...
classzz or czzwallet by default.  However, configuration options are provided to
...
In contrast, the websocket-based JSON-RPC interface provided by classzz and
czzwallet only uses a single connection that remains open and allows
...
All notifications provided by classzz require registration to opt-in.  For example,
...
example, the DebugLevel call is an extension only provided by classzz (and
czzwallet passthrough).  Therefore if you call one of these commands against
...
Also, it is important to realize that classzz intentionally separates the wallet
functionality into a separate process named czzwallet.  This means if you are
connected to the classzz RPC server directly, only the RPCs which are related to
chain services will be available.  Depending on your application, you might only
need chain-related RPCs.  In contrast, czzwallet provides pass through treatment
...
The following full-blown client examples are in the examples directory:
 ...
 - czzdwebsockets
   ...
   block count
 - czzwalletwebsockets
   Connects to a czzwallet RPC server using TLS-secured websockets, registers
