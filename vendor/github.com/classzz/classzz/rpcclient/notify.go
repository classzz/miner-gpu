...
type NotificationHandlers struct {
	...
	OnRecvTx func(transaction *czzutil.Tx, details *btcjson.BlockDetails)
	...
	//
	// Deprecated: Use OnRelevantTxAccepted instead.
	OnRedeemingTx func(transaction *czzutil.Tx, details *btcjson.BlockDetails)

	// OnRelevantTxAccepted is invoked when an unmined transaction passes
	// the client's transaction filter.
	//
	// NOTE: This is a bourbaki-czz extension ported from
	...
	// result of a rescan request, due to how classzz may send various rescan
	...
	// made to register for the notification and the function is non-nil.
	OnTxAccepted func(hash *chainhash.Hash, amount czzutil.Amount)
	...
	// such as czzwallet.
	OnAccountBalance func(account string, balance czzutil.Amount, confirmed bool)
...
func (c *Client) handleNotification(ntfn *rawNotification) {
	...
	case btcjson.BtcdConnectedNtfnMethod:
		...
		if err != nil {
			log.Warnf("Received invalid classzz connected "+
				"notification: %v", err)
			return
		}
...
//
// NOTE: This is a classzz extension ported from github.com/decred/dcrrpcclient
// and requires a websocket connection.
func parseFilteredBlockConnectedParams(params []json.RawMessage) (int32,
	*wire.BlockHeader, []*czzutil.Tx, error) {
	...
	transactions := make([]*czzutil.Tx, len(hexTransactions))
	for i, hexTx := range hexTransactions {
		...
		transactions[i], err = czzutil.NewTxFromBytes(transaction)
		if err != nil {
			return 0, nil, nil, err
		}
	}
...
// notifications.
func parseChainTxNtfnParams(params []json.RawMessage) (*czzutil.Tx,
	*btcjson.BlockDetails, error) {
	...
	return czzutil.NewTx(&msgTx), block, nil
}
...
func parseTxAcceptedNtfnParams(params []json.RawMessage) (*chainhash.Hash,
	...
	// Bounds check amount.
	amt, err := czzutil.NewAmount(famt)
	if err != nil {
		return nil, 0, err
	}
...
// parseBchdConnectedNtfnParams parses out the connection status of classzz
// and czzwallet from the parameters of a czzdconnected notification.
...
func parseAccountBalanceNtfnParams(params []json.RawMessage) (account string,
	balance czzutil.Amount, confirmed bool, err error) {
	...
	// Bounds check amount.
	bal, err := czzutil.NewAmount(fbal)
	if err != nil {
		return "", 0, false, err
	}
...
//
// NOTE: This is a classzz extension and requires a websocket connection.
...
//
// Deprecated: Use LoadTxFilterAsync instead.
func (c *Client) NotifyReceivedAsync(addresses []czzutil.Address) FutureNotifyReceivedResult {
	// Not supported in HTTP POST mode.
	if c.config.HTTPPostMode {
		return newFutureError(ErrWebsocketsRequired)
	}
...
//
// Deprecated: Use LoadTxFilter instead.
func (c *Client) NotifyReceived(addresses []czzutil.Address) error {
...
func (c *Client) RescanEndBlockAsync(startBlock *chainhash.Hash,
	addresses []czzutil.Address, outpoints []*wire.OutPoint,
...
// NOTE: This is a classzz extension ported from github.com/decred/dcrrpcclient
// and requires a websocket connection.
func (c *Client) LoadTxFilterAsync(reload bool, addresses []czzutil.Address,
...
// NOTE: This is a classzz extension ported from github.com/decred/dcrrpcclient
// and requires a websocket connection.
func (c *Client) LoadTxFilter(reload bool, addresses []czzutil.Address, outPoints []wire.OutPoint) error {
