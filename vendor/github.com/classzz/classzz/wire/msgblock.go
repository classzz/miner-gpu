...
func (msg *MsgBlock) CzzDecode(r io.Reader, pver uint32, enc MessageEncoding) error {
	...
	if txCount > uint64(maxTxPerBlock()) {
		...
		return messageError("MsgBlock.CzzDecode", str)
	}
...
// Version field in the block.  This function differs from CzzDecode in that
// CzzDecode decodes from the bitcoin wire protocol as it was sent across the
...
func (msg *MsgBlock) Deserialize(r io.Reader) error {
	...
	// a result, make use of CzzDecode.
	return msg.CzzDecode(r, 0, BaseEncoding)
}
...
func (msg *MsgBlock) CzzEncode(w io.Writer, pver uint32, enc MessageEncoding) error {
...
// This function differs from CzzEncode in that CzzEncode encodes the block to
...
func (msg *MsgBlock) Serialize(w io.Writer) error {
	// At the current time, there is no difference between the wire encoding
	// at protocol version 0 and the stable long-term storage format.  As
	// a result, make use of CzzEncode.
	return msg.CzzEncode(w, 0, BaseEncoding)
}
