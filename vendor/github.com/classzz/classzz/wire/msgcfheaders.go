...
func (msg *MsgCFHeaders) CzzDecode(r io.Reader, pver uint32, _ MessageEncoding) error {
	// Read filter type
	...
	if count > MaxCFHeadersPerMsg {
		str := fmt.Sprintf("too many committed filter headers for "+
			...
			MaxBlockHeadersPerMsg)
		return messageError("MsgCFHeaders.CzzDecode", str)
	}
...
// This is part of the Message interface implementation.
func (msg *MsgCFHeaders) CzzEncode(w io.Writer, pver uint32, _ MessageEncoding) error {
	// Write filter type
	...
	if count > MaxCFHeadersPerMsg {
		str := fmt.Sprintf("too many committed filter headers for "+
			"message [count %v, max %v]", count,
			MaxBlockHeadersPerMsg)
		return messageError("MsgCFHeaders.CzzEncode", str)
	}
...
// differs from CzzDecode in that CzzDecode decodes from the bitcoin wire
