...
type Message interface {
	CzzDecode(io.Reader, uint32, MessageEncoding) error
	CzzEncode(io.Writer, uint32, MessageEncoding) error
	Command() string
...
func WriteMessageN(w io.Writer, msg Message, pver uint32, czznet BitcoinNet) (int, error) {
	return WriteMessageWithEncodingN(w, msg, pver, czznet, BaseEncoding)
}
...
func WriteMessage(w io.Writer, msg Message, pver uint32, czznet BitcoinNet) error {
	_, err := WriteMessageN(w, msg, pver, czznet)
	return err
}
...
func WriteMessageWithEncodingN(w io.Writer, msg Message, pver uint32,
	czznet BitcoinNet, encoding MessageEncoding) (int, error) {
	...
	var bw bytes.Buffer
	err := msg.CzzEncode(&bw, pver, encoding)
	if err != nil {
...
func ReadMessageWithEncodingN(r io.Reader, pver uint32, czznet BitcoinNet,
	...
	// MsgVersion CzzDecode function requires it.
	pr := bytes.NewBuffer(payload)
	err = msg.CzzDecode(pr, pver, enc)
	if err != nil {
		return totalBytes, nil, nil, err
	}
...
func ReadMessageN(r io.Reader, pver uint32, czznet BitcoinNet) (int, Message, []byte, error) {
	return ReadMessageWithEncodingN(r, pver, czznet, BaseEncoding)
}
...
func ReadMessage(r io.Reader, pver uint32, czznet BitcoinNet) (Message, []byte, error) {
	_, msg, buf, err := ReadMessageN(r, pver, czznet)
	return msg, buf, err
}
