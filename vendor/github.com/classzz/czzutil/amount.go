...
// monetary amount.
const (
	AmountMegaCZZ  AmountUnit = 6
	AmountKiloCZZ  AmountUnit = 3
	AmountCZZ      AmountUnit = 0
	AmountMilliCZZ AmountUnit = -3
	AmountMicroCZZ AmountUnit = -6
	AmountSatoshi  AmountUnit = -8
)

// String returns the unit as a string.  For recognized units, the SI
// prefix is used, or "Satoshi" for the base unit.  For all unrecognized
// units, "1eN CZZ" is returned, where N is the AmountUnit.
func (u AmountUnit) String() string {
	...
	case AmountSatoshi:
		return "Satoshi"
	default:
		return "1e" + strconv.FormatInt(int64(u), 10) + " CZZ"
	}
}
...
//
// NewAmount is for specifically for converting CZZ to Satoshi.
// For creating a new Amount with an int64 value which denotes a quantity of Satoshi,
// do a simple type conversion from type int64 to Amount.
// See GoDoc for example: http://godoc.org/github.com/gcash/CZZutil#example-Amount
...
// ToCZZ is the equivalent of calling ToUnit with AmountCZZ.
func (a Amount) ToCZZ() float64 {
	return a.ToUnit(AmountCZZ)
}
...
// String is the equivalent of calling Format with AmountCZZ.
func (a Amount) String() string {
	return a.Format(AmountCZZ)
}
